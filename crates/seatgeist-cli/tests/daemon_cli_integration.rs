...
impl DaemonFixture {
    ...
    fn cli_json(&self, args: &[&str]) -> Result<DaemonResponse> {
        let output = Command::new(env!("CARGO_BIN_EXE_seatgeist-cli"))
            .arg("--socket")
            .arg(&self.socket)
            .args(args)
            .output()
            .with_context(|| format!("run seatgeist-cli {}", args.join(" ")))?;
    ...
    fn cli_output(&self, args: &[&str]) -> Result<Output> {
        Command::new(env!("CARGO_BIN_EXE_seatgeist-cli"))
            .arg("--socket")
            .arg(&self.socket)
            .args(args)
            .output()
...
fn cli_talks_to_real_daemon_for_status_commands() -> Result<()> {
    ...
    assert!(capabilities.contains(&BackendCapability::DaemonComputerUseReadiness));
...
fn cli_routes_atspi_text_attribute_validation_to_daemon() -> Result<()> {
    ...
    assert!(!entry.ok);
    assert_eq!(entry.safety_class, Some(libseatgeist::SafetyClass::Observe));
    Ok(())
}
...
fn cli_replays_trace_against_real_daemon() -> Result<()> {
    ...
    assert_eq!(
        report["steps"][6]["response_type"],
        "accessibility_quality_status"
    );
    assert_eq!(report["steps"][6]["ok"], true);
    assert_eq!(report["steps"][7]["method"], "kwin_bridge_status");
    assert_eq!(report["steps"][7]["response_type"], "kwin_bridge_status");
    assert_eq!(report["steps"][7]["ok"], true);
    assert_eq!(report["steps"][8]["method"], "uinput_status");
    assert_eq!(report["steps"][8]["response_type"], "uinput_status");
    assert_eq!(report["steps"][8]["ok"], true);
    assert_eq!(report["steps"][9]["method"], "capture_backend_status");
    assert_eq!(
        report["steps"][9]["response_type"],
        "capture_backend_status"
    );
    assert_eq!(report["steps"][9]["ok"], true);
    assert_eq!(report["steps"][10]["method"], "clipboard_backend_status");
    assert_eq!(
        report["steps"][10]["response_type"],
        "clipboard_backend_status"
    );
    assert_eq!(report["steps"][10]["ok"], true);
    assert_eq!(report["steps"][11]["method"], "input_backend_status");
    assert_eq!(report["steps"][11]["response_type"], "input_backend_status");
    assert_eq!(report["steps"][11]["ok"], true);

    let journal = daemon.cli_json(&["journal", "tail", "--limit", "14"])?;
    let DaemonResponse::Journal(entries) = journal else {
        bail!("expected journal response, got {journal:?}");
    };
    assert_methods(
...
fn cli_replays_policy_denial_trace_against_real_daemon() -> Result<()> {
    let daemon = DaemonFixture::start()?;
    let denied_screenshot = Path::new("/tmp/seatgeist-denied-full-resolution.png");
...
fn cli_replays_semantic_denial_trace_against_real_daemon() -> Result<()> {
    ...
    assert!(
        entries
            .iter()
            .all(|entry| entry.safety_class == Some(libseatgeist::SafetyClass::ControlSemantic))
    );
    let journal_json = serde_json::to_string(&entries).context("serialize semantic journal")?;
    assert!(
        !journal_json.contains("seatgeist-should-not-set"),
...
fn cli_replay_directory_rejects_empty_trace_set() -> Result<()> {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_seatgeist-cli"))
        ...
        .output()
        .context("run seatgeist-cli trace replay --dir")?;
    assert!(!output.status.success());
    assert!(output.stdout.is_empty());
...
fn cli_validates_trace_without_daemon() -> Result<()> {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_seatgeist-cli"))
        ...
        .output()
        .context("run seatgeist-cli trace validate")?;
    ...
    assert_eq!(report["steps"][8]["method"], "uinput_status");
    assert_eq!(report["steps"][8]["expect_json_count"], 2);
    assert_eq!(report["steps"][9]["method"], "capture_backend_status");
    assert_eq!(report["steps"][9]["expect_json_count"], 4);
    assert_eq!(report["steps"][10]["method"], "clipboard_backend_status");
    assert_eq!(report["steps"][10]["expect_json_count"], 6);
    assert_eq!(report["steps"][11]["method"], "input_backend_status");
    assert_eq!(report["steps"][11]["expect_json_count"], 3);
    assert_eq!(
        report["steps"][12]["method"],
...
fn cli_validates_all_checked_in_traces_without_daemon() -> Result<()> {
    ...
    for trace_path in traces {
        ...
        let output = Command::new(env!("CARGO_BIN_EXE_seatgeist-cli"))
            .args(["trace", "validate", "--file", &trace_arg])
            .output()
            .with_context(|| format!("run seatgeist-cli trace validate for {trace_arg}"))?;
        require_success(&["trace", "validate"], &output)?;
        let report: serde_json::Value =
            serde_json::from_slice(&output.stdout).context("parse trace validation report")?;
        assert_eq!(report["type"], "trace_validation");
...
fn cli_validates_trace_directory_without_daemon() -> Result<()> {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_seatgeist-cli"))
        ...
        .output()
        .context("run seatgeist-cli trace validate --dir")?;
...
fn cli_writes_expiring_approval_grant() -> Result<()> {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_seatgeist-cli"))
        ...
        .arg("60000")
        .output()
        .context("run seatgeist-cli approve")?;
...
fn require_success(args: &[&str], output: &Output) -> Result<()> {
    ...
    bail!(
        "seatgeist-cli {} failed with status {:?}\nstdout:\n{}\nstderr:\n{}",
        args.join(" "),
        output.status.code(),
...
fn daemon_binary() -> Result<PathBuf> {
    let candidate = PathBuf::from(env!("CARGO_BIN_EXE_seatgeist-cli")).with_file_name("seatgeistd");
