...
4. Prefer `plasma.a11y_find`, `plasma.click_button`, `plasma.set_text_field`, `plasma.select_menu`, `plasma.activate_tab`, `plasma.toggle_check`, and `plasma.set_value` for repeatable UI operations; use `plasma.a11y_insert_text` or `plasma.a11y_delete_text` only for known non-sensitive `EditableText` offset editing.
