...
4. Use `plasma.screenshot` or `plasma.screenshot_tile` for display/capture issues, keeping screenshots bounded by default.
5. Use `plasma.a11y_focused_tree` and `plasma.a11y_find` for accessibility or semantic-action issues.
6. Use `plasma.journal_tail` with method/success filters to distinguish policy denials from backend failures.
7. Report exact backend names, command errors, policy decisions, and setup hints. Suggest system changes only after the diagnostics show they are needed.
