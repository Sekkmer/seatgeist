...
verify: fmt check test clippy validate-plugin validate-traces smoke-trace-replay gui-eval-control-safety
...
smoke-trace-replay:
	...
	jq -e '.type == "trace_validation_set" and .trace_count >= 4 and .step_count >= 19 and any(.traces[]; (.file | endswith("status-smoke.json")) and .step_count == 5 and any(.steps[]; .method == "safety_status")) and any(.traces[]; (.file | endswith("policy-denials-smoke.json")) and .step_count == 3 and all(.steps[]; .expect_response_type == "error" and .expect_ok == false and (.expect_error_contains | type == "string"))) and any(.traces[]; (.file | endswith("input-denials-smoke.json")) and .step_count == 6 and all(.steps[]; .expect_response_type == "error" and .expect_ok == false and (.expect_error_contains | test("Control(Keyboard|Pointer)")))) and any(.traces[]; (.file | endswith("panic-stop-smoke.json")) and .step_count == 5 and all(.steps[]; .expect_json_count == 1))' "$$validate_out" >/dev/null
	target/debug/plasma-pilotd --socket "$$socket" --journal "$$journal" >"$$log" 2>&1 &
	pid=$$!
	cleanup() {
		kill "$$pid" 2>/dev/null || true
	...
	jq -e '.type == "trace_replay_set" and .trace_count >= 4 and .step_count >= 19 and any(.traces[]; (.file | endswith("status-smoke.json")) and (.steps | length) == 5 and all(.steps[]; .ok == true) and any(.steps[]; .method == "safety_status")) and any(.traces[]; (.file | endswith("policy-denials-smoke.json")) and (.steps | length) == 3 and all(.steps[]; .response_type == "error" and .ok == false) and any(.steps[]; .method == "focus_window")) and any(.traces[]; (.file | endswith("input-denials-smoke.json")) and (.steps | length) == 6 and all(.steps[]; .response_type == "error" and .ok == false) and any(.steps[]; .method == "type_text") and any(.steps[]; .method == "click_pointer")) and any(.traces[]; (.file | endswith("panic-stop-smoke.json")) and (.steps | length) == 5 and all(.steps[]; .response_type == "panic_stop" and .ok == true) and any(.steps[]; .method == "set_panic_stop"))' "$$replay_out" >/dev/null
