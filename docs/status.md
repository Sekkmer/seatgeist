...
- `make smoke-atspi` validates the daemon-backed focused accessibility tree and accessibility find paths and confirms both requests are journaled.
- Pointer/keyboard input, AT-SPI invoke/set-text, text/value extraction, MCP wait/change tools, portal/KDE-native clipboard fallback, portal/KWin-native capture backends, persistent active-window bridge installation checks, and richer journal filtering remain future work.
