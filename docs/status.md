...
- `make smoke-atspi` validates the daemon-backed focused accessibility tree path and confirms the request is journaled.
- Pointer/keyboard input, AT-SPI find/invoke/set-text, MCP wait/change tools, portal/KDE-native clipboard fallback, portal/KWin-native capture backends, persistent active-window bridge installation checks, and richer journal filtering remain future work.
