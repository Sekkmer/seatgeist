...
- `make smoke-atspi` validates the daemon-backed focused accessibility tree, accessibility find paths, and deny-by-default accessibility invoke path, and confirms the requests are journaled.
- Pointer/keyboard input, AT-SPI set-text, MCP wait/change tools, portal/KDE-native clipboard fallback, portal/KWin-native capture backends, persistent active-window bridge installation checks, and richer journal filtering remain future work.
