...
`seatgeist-cli atspi quality-status` and MCP
`seatgeist.a11y_quality_status` also count same-user
`at-spi2-registryd` processes through read-only `/proc` metadata. More than one
is reported as an extra-registry setup hint because it can indicate stale
accessibility-bus generations after an interrupted desktop session. This is a
diagnostic only; Seatgeist does not terminate or restart accessibility
services.
...
The action path uses a daemon-side two-second long poll and an immediate
re-arm after each successful reply. A slower script timer is retained only as
a stalled-call or absent-service watchdog. It permits concurrent daemon
callers through opaque action ids and fails closed after a bounded
acknowledgment timeout rather than falling back to pointer dragging or a
private KWin DBus method.
...
The daemon requests `org.seatgeist.KWinBridge` without replacement flags. If
another owner holds the name or the session bus is temporarily unavailable,
window actions fail closed and ownership recovery backs off exponentially up
to 30 seconds with small jitter. Bridge status exposes bounded retry metadata.
The WindowsRunner/KWin query path remains the documented read-only window
metadata fallback; it does not become a control fallback.
...
The bridge also exposes exact `close_window` through KWin's script
`Window.closeWindow()` API. The daemon identifies the target by KWin UUID and
waits for that exact UUID to disappear before reporting success. Because this
is a KWin scripting API whose availability can vary by compositor version, the
bridge advertises a separate `window_close_supported` capability. Missing API,
missing capability, or lost bridge acknowledgement fails closed; Seatgeist
does not substitute `Alt+F4`, `Ctrl+W`, or another keyboard shortcut.
