...
Current keyboard input implementation uses a localized Linux uinput backend in `plasma-pilot-uinput`. It creates a short-lived virtual keyboard through `/dev/uinput`, uses `UI_DEV_SETUP`, supports US evdev key positions for ASCII text plus newline/tab, and supports named key combos such as `Ctrl+L`, `Alt+F4`, and `Super+Space`. The daemon exposes this only through policy-gated `ControlKeyboard` requests, active panic-stop blocks it, and compact summaries report text length or key count only. The daemon reports `keyboard_input` capability only when `/dev/uinput` can be opened read/write. Absolute pointer positioning and click/scroll tools remain future work because they need explicit coordinate-space mapping and portal/libei/uinput fallback decisions.
