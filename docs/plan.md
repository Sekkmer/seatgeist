...
- Add denial-source and accessibility-quality diagnostics. Current status: safety, policy, backend, and session status are visible through compact CLI/MCP outputs, and `docs/mac-computer-use-research.md` captures the user-reported failure classes that should drive the next diagnostic slices: app-policy vs OS/session/backend failures, flat or empty AT-SPI trees, dynamic UI identifiers, and stale/background execution symptoms.
