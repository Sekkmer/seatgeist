...
- Add replayable action traces. Current status: `ReplayTrace` stores daemon requests with expected response metadata, including optional error-message substring expectations for fail-closed paths. `plasma-pilot-cli trace validate --file <path>` checks trace structure, version, and expected response types without contacting the daemon. `plasma-pilot-cli trace replay --file <path>` replays each step through the daemon so policy checks and journaling still apply; replay mismatches report the step index, label, method, and mismatch detail. `examples/traces/status-smoke.json` plus `examples/traces/policy-denials-smoke.json` provide checked-in safe replay fixtures covered by real CLI integration tests and `make smoke-trace-replay`.
