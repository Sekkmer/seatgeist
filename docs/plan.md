...
- Add integration tests for CLI, MCP, and daemon protocol. Current status: daemon core protocol, low-risk CLI status commands, approval writing, trace replay, panic-stop toggling, desktop-independent AT-SPI text-attribute CLI validation, and real MCP stdio initialize/tool-call/journal behavior have Rust integration tests; GUI/desktop CLI coverage remains in smoke targets. `make smoke-mcp` validates the current AT-SPI MCP tool list, including text attributes and offset-edit operations, and checks daemon-backed MCP tool error propagation for text-attribute validation. `make smoke-trace-replay` replays the checked-in status-only trace against a private daemon and verifies trace output plus journal evidence.
...
- Add replayable action traces. Current status: `ReplayTrace` stores daemon requests with expected response metadata, `plasma-pilot-cli trace replay --file <path>` replays each step through the daemon so policy checks and journaling still apply, replay mismatches report the step index, label, method, and mismatch detail, and `examples/traces/status-smoke.json` provides a checked-in status-only replay fixture covered by the real CLI integration test and `make smoke-trace-replay`.
