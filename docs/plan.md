...
Checklist tracker: `docs/tracker.md` records done, partial, and remaining work using `[x]`, `[~]`, and `[ ]` against the full Seatgeist project scope.
...
Build **Seatgeist**, a local desktop-control substrate that lets Codex CLI safely observe and operate the user's Linux desktop through a Codex plugin, MCP tools, skills, hooks, a CLI, and a privileged local daemon. The first supported backend is KDE Plasma 6 on Wayland.

## 2. Why this project exists

Official Codex Computer Use is useful because it lets an agent see, click, type, and use normal applications. Linux/KDE does not yet have an equivalent first-class path in Codex app, but Codex CLI can already use local tools through MCP and reusable workflows through skills. Seatgeist fills that gap first for KDE Plasma 6 by exposing desktop observation, input, window state, clipboard, and semantic UI information as controlled local tools.
...
KDE Plasma remains the only implementation target for the current project. The public project name is `Seatgeist`, and binaries, crates, service names, runtime paths, plugin config, and MCP tools use the `seatgeist-*` / `seatgeist.*` identity. Keep the core protocol, policy, and backend traits neutral enough that a future GNOME, wlroots/Sway, or X11 backend could implement the same contracts without changing the model-facing tool semantics.

## 2.1 Fact-check notes and source anchors

The plan was checked on 2026-07-04 against current public documentation:

- OpenAI Codex docs: Codex Computer Use is documented for macOS and Windows in the Codex app, not Linux/KDE. This validates Seatgeist as a Linux/KDE-specific local integration rather than a duplicate of an existing official Linux Computer Use path. Source: <https://developers.openai.com/codex/app/computer-use>
...
- Mac computer-use field research: current user reports and third-party tools highlight recurring gaps around permission/app-approval confusion, accessibility-tree quality, screenshot cost, dynamic UI identifiers, regional/device availability, background throttling, and security/data-leakage risk. Seatgeist tracks these in `docs/mac-computer-use-research.md` and turns them into KDE preflight, policy, semantic-selector, and eval requirements.
...
- KDE KWin review history: KWin implemented the `org.kde.KeyboardLayouts` DBus interface so keyboard-layout DBus clients could also work on Wayland. Seatgeist treats this as a best-effort KDE surface and falls back to `kxkbrc`/xkbcommon when unavailable. Source: <https://phabricator.kde.org/D4323>
- Naming search: exact-name web searches for `Seatgeist` on 2026-07-05 did not show an obvious existing software/project collision; most visible near-matches were for `Zeitgeist`, including the older freedesktop/GNOME activity service. This is not a trademark clearance and must be followed by registry, domain, and legal checks before public release.
...
Project name: **Seatgeist**

First backend target: **KDE Plasma 6 Wayland**

Preferred crate and binary names:

```text
seatgeistd        # local daemon / service
seatgeist-mcp     # MCP stdio server used by Codex
seatgeist-cli     # diagnostics and manual control CLI
seatgeist-kwin    # KWin integration package, script/plugin/backend
seatgeist-plugin  # Codex plugin bundle
libseatgeist      # shared Rust library crate
```
...
The public-facing spelling should be `Seatgeist`; package names should use `seatgeist-*`.
...
Seatgeist Codex Plugin
  ...
  v
seatgeist-mcp              stdio MCP server
  |
  | Unix socket / local RPC
  v
seatgeistd                 local daemon
  |-- policy engine
  |-- action journal
...
├── crates/
│   ├── libseatgeist/     # shared protocol/types
│   ├── seatgeistd/
│   ├── seatgeist-cli/
│   ├── seatgeist-mcp/
│   ├── seatgeist-policy/
...
│   └── seatgeistd.socket
├── udev/
│   └── 99-seatgeist-uinput.rules
├── polkit/
│   └── org.seatgeist.policy
├── docs/
│   ├── plan.md
│   ├── status.md
│   ├── architecture.md
...
seatgeist-cli screenshot [--output /tmp/screen.png]
seatgeist-cli windows
seatgeist-cli active-window
seatgeist-cli focus --window <id>
seatgeist-cli input click-pointer --x <x> --y <y> --coordinate-space physical-pixel --button left
seatgeist-cli input drag-pointer --from-x <x> --from-y <y> --to-x <x> --to-y <y> --coordinate-space physical-pixel
seatgeist-cli input type-text "hello"
seatgeist-cli input key-combo Ctrl+L
seatgeist-cli clipboard get
seatgeist-cli clipboard set "text"
seatgeist-cli atspi tree --focused
seatgeist-cli journal tail
```
...
3. Use the packaged `kwin/seatgeist-bridge` script for active-window updates; it reads `workspace.activeWindow` and calls the daemon's `org.seatgeist.KWinBridge1.UpdateActiveWindow` method over DBus.
...
- [x] Produce screenshot change thresholds for `seatgeist.wait_for_change`. Current status: the daemon polls bounded screenshots, computes normalized RGB deltas, and returns changed/timed-out/captures/elapsed/timeout/interval/score metadata plus latest screenshot metadata, so a no-change watchdog timeout is distinguishable from a failed command or backend error.
...
```json
{
  ...
  "mcpServers": "./.mcp.json",
  "interface": {
    "displayName": "Seatgeist",
    "shortDescription": "Use KDE Plasma desktop apps from Codex through local MCP tools.",
    "longDescription": "Seatgeist exposes a local KDE Plasma desktop-control runtime to Codex. It provides screenshots, window state, input, clipboard, accessibility metadata, safety policy, and audit logging through MCP tools and reusable skills.",
...
```json
{
  "mcp_servers": {
    "seatgeist": {
      ...
      "args": ["--stdio"],
      "env": {
        "SEATGEIST_SOCKET": "${XDG_RUNTIME_DIR}/seatgeist/seatgeistd.sock"
      }
    }
  }
}
...
```text
Use KDE Plasma desktop computer-use tools through Seatgeist MCP when a task requires seeing, clicking, typing, controlling windows, reading screenshots, or operating GUI applications on Linux/KDE.
```
...
- After Seatgeist actions, append a local audit record.
- On `Stop`, summarize actions taken and any denied actions.

Current hook implementation: `plugin/hooks/hooks.json` enables one conservative Codex `Stop` command hook. After Codex's normal `/hooks` trust review, it resolves and runs the bundled `seatgeist_audit_summary.py` from the current checkout, active Codex plugin cache, or local Seatgeist checkout, then writes `target/seatgeist-hook-audit/latest.json`. The hook is fail-open, ignores prompt/hook stdin, and records only repo status, HEAD/branch, recent compact Seatgeist journal metadata, failure/control/unguarded-control counts, method, safety-class, and client counts, and compact active-window context. `make validate-plugin` verifies the hook command, timeout, script import, and audit aggregation behavior.
...
- Unix socket must be under `$XDG_RUNTIME_DIR/seatgeist/` with mode `0600` by default.
...
- `seatgeist-cli --help` works.
- `seatgeist-mcp --help` works.
...
- Implement `seatgeistd` with async Unix socket server.
...
- `systemctl --user start seatgeistd` works.
- `seatgeist-cli doctor` reports daemon status.
- Daemon refuses clients from wrong UID or unsafe socket permissions.
- `seatgeist-cli capabilities` and `seatgeist-cli policy-status` return daemon responses.
...
- [x] Save screenshots to `$XDG_RUNTIME_DIR/seatgeist/screenshots/` by default. Current status: direct CLI `screenshot`, `screenshot-tile`, and `wait-for-change` commands plus MCP `seatgeist.screenshot`, `seatgeist.screenshot_tile`, and `seatgeist.wait_for_change` accept explicit output paths, but when omitted they write timestamped PNGs under the Seatgeist runtime screenshot directory.
- Add `seatgeist-cli screenshot`.
...
- [x] Implement initial keyboard input backend using uinput. Current status: `seatgeist-uinput` creates a short-lived `/dev/uinput` virtual keyboard with `UI_DEV_SETUP`; daemon/CLI/MCP expose `type_text` and `key_combo` as policy-gated `ControlKeyboard`.
- [x] Create virtual pointer device and absolute/relative pointer mapping. Current status: `seatgeist-uinput` creates a short-lived `/dev/uinput` virtual pointer with absolute X/Y axes and relative wheel axes; the daemon maps physical desktop coordinates into the absolute input range.
...
- [x] Implement active window query bridge. Current status: daemon DBus receiver and packaged KWin script exist; installation is explicit through `make install-kwin-script`. The script combines immediate KWin window-event updates with a bounded two-second snapshot heartbeat so daemon-only restarts recover without a focus change; a mocked runtime test verifies startup, heartbeat, interval bounds, and the timer-unavailable fallback. The modular installer also performs a targeted live-script reload after package updates, with tested offline deferral, because a general KWin reconfigure does not replace an already-running JavaScript instance reliably.
...
- [x] Implement focus window. Current status: `seatgeist-cli focus --window <id>` uses KWin `WindowsRunner.Run` and is policy-gated as `ControlSemantic`; default policy fails closed without a matching approval grant, while `seatgeist-cli approve` plus daemon `--approval-file` enables method-scoped local use and `seatgeistd --allow-control` remains a broad explicit local mode.
- [x] Create a KWin script exposing active-window metadata through DBus.

Acceptance criteria:

- CLI lists open windows with stable ids. Initial implementation is present through `seatgeist-cli windows`.
...
Goal: Codex CLI can use Seatgeist tools.
...
- `seatgeist-mcp --stdio` starts and responds to tool calls. `make smoke-mcp` validates initialize, tools/list, and a daemon-backed health tool call.
...
- [x] Finalize `plugin/.mcp.json`. Current status: bundled MCP config points at `seatgeist-mcp --stdio`, with daemon socket resolution handled by the MCP/daemon defaults or `SEATGEIST_SOCKET`.
- [x] Write the four skills. Current status: the skills describe current `seatgeist.*` MCP tools, safety guards, observation flow, browser debugging, GUI testing, and desktop triage.
- [x] Create hook skeleton. Current status: `plugin/hooks/hooks.json` now enables one Codex `Stop` command hook that writes a fail-open local audit summary under `target/seatgeist-hook-audit/latest.json`; the summary includes repo status, recent compact journal entries, method/safety-class/client counts, failure examples, unguarded-control examples, and last active-window context. Codex still requires the normal `/hooks` trust review before non-managed plugin hooks run.
...
- [x] Add KDE/portal-native fallback and explicit provenance selection if `wl-copy`/`wl-paste` is unavailable. Current status: daemon clipboard reads/writes prefer `wl-clipboard` and fall back to KDE Klipper DBus (`org.kde.klipper`) when the Wayland commands are unavailable; both production adapters now implement the shared async `ClipboardBackend` trait in the focused daemon clipboard module. `ClipboardText` responses and clipboard-set action summaries include backend provenance. `seatgeist-cli clipboard status` and MCP `seatgeist.clipboard_status` report `wl-paste`, `wl-copy`, and KDE Klipper DBus availability plus selected read/write backend names without reading clipboard contents.
...
- [x] Implement copy text where `org.a11y.atspi.EditableText` is supported. Current status: CLI/MCP expose policy-gated `a11y_copy_text` with active-window guards, range validation, and offset-only summaries; Seatgeist does not read copied clipboard contents.
- [x] Implement cut text where `org.a11y.atspi.EditableText` is supported. Current status: CLI/MCP expose policy-gated `a11y_cut_text` with active-window guards, range validation, and offset-only summaries; Seatgeist does not read cut clipboard contents.
- [x] Implement paste text where `org.a11y.atspi.EditableText` is supported. Current status: CLI/MCP expose policy-gated `a11y_paste_text` with active-window guards, offset validation, and offset-only summaries; Seatgeist does not read clipboard contents for this operation.
...
Eval 17: focused AT-SPI tree read remains safe and journaled. Current status: `make gui-eval-a11y-focused-tree` starts a private daemon, uses `accessibility_quality_status` as a readiness probe, skips cleanly when a focused AT-SPI tree is unavailable, and otherwise checks compact focused-tree metadata plus journal evidence without invoking accessibility control, screenshots, portal prompts, clipboard reads, or input. This eval is included in `make verify`.
Eval 18: AT-SPI find remains safe and journaled. Current status: `make gui-eval-a11y-find` starts a private daemon, uses `accessibility_quality_status` as a readiness probe, skips cleanly when AT-SPI is unavailable or the find call returns a journaled `AccessibilityUnavailable` runtime error, and otherwise checks bounded application-role search metadata plus journal evidence without invoking accessibility control, screenshots, portal prompts, clipboard reads, or input. This eval is included in `make verify`.
Eval 19: AT-SPI text-attribute reads remain safe and journaled. Current status: `make gui-eval-a11y-text-attributes` starts a private daemon, uses `accessibility_quality_status` plus bounded text-role find as readiness probes, skips cleanly when AT-SPI, AT-SPI find, text nodes, or text-attribute reads are unavailable, and otherwise checks compact `accessibility_text_attributes` metadata plus journal evidence without invoking accessibility control, screenshots, portal prompts, clipboard reads, or input. This eval is included in `make verify`.
Eval 20: high-level semantic control fails closed by default. Current status: `make gui-eval-semantic-denied` starts a private daemon without approval grants and verifies `click_button`, `set_text_field`, `focus_text_field`, `activate_tab`, `activate_link`, `toggle_check`, `set_value`, `select_item`, and `select_menu` all return policy denials and journal `ControlSemantic` `PolicyPromptRequired` failures before AT-SPI backend control can execute. This eval is included in `make verify`.
Eval 21: direct AT-SPI control fails closed by default. Current status: `make gui-eval-a11y-control-denied` starts a private daemon without approval grants and verifies `accessibility_invoke`, `accessibility_set_text`, `accessibility_insert_text`, `accessibility_delete_text`, `accessibility_copy_text`, `accessibility_cut_text`, `accessibility_paste_text`, `accessibility_set_caret`, and `accessibility_set_selection` all return policy denials and journal `ControlSemantic` `PolicyPromptRequired` failures before AT-SPI backend control can execute. This eval is included in `make verify`.
Eval 22: raw keyboard and pointer input fail closed by default. Current status: `make gui-eval-input-denied` starts a private daemon without approval grants and verifies `type_text`, `key_combo`, `move_pointer`, `click_pointer`, `drag_pointer`, and `scroll_pointer` all return policy denials and journal `ControlKeyboard` or `ControlPointer` `PolicyPromptRequired` failures before uinput/EIS backend control can execute. This eval is included in `make verify`.
```
...
```text
Implement Phase 0 and Phase 1 of docs/plan.md for Seatgeist.
...
- [~] Add release hardening. Current status: the canonical GitHub repository is private, `.github` is intentionally absent, and GitHub Actions remains disabled until public release preparation. `docs/release-checklist.md` records that transition as a blocker, `docs/unsupported-paths.md` documents unsupported public paths, and `make validate-release` enforces the dual license files, canonical repository metadata, and private-state CI guardrail. `make verify-release-artifacts` builds a local binary/plugin archive, tracked-file source archive, manifest, and checksums, then verifies manifest coherence, checksums, bundle contents, and source contents. `make verify-release-install` extracts the generated bundle outside the working tree, runs packaged binary version checks, and validates packaged plugin/install assets. `make sign-release-artifacts` can create detached GPG signatures and a signature checksum manifest when `SEATGEIST_RELEASE_SIGNING_KEY` is set, and `make verify-release-signatures` verifies the generated signatures. GUI eval scripts now write explicit `seatgeist_eval_evidence` JSON records on success. `make check-public-name` writes repeatable exact-name public registry evidence for crates.io, npm, PyPI, and GitHub repository names. `make release-readiness` reports current blockers across metadata, checklist, name-collision evidence, artifacts, signatures, and opt-in live eval evidence, requiring matching `evidence.json` pass records for release evals; `scripts/release-readiness.py --strict` is the final local no-blocker audit before upload. The reviewed public CI workflow, signed release tags, public uploads, and broader manual release evidence remain incomplete.

## 20. Definition of done for v0.1

v0.1 is complete when:

- Seatgeist daemon runs under the user session.
