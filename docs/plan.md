...
- [x] Add udev/polkit/systemd instructions. Current status: skeleton files exist, `docs/uinput-setup.md` documents the optional udev rule, user service setup, current polkit placeholder state, and `input uinput-status` diagnostics. `make validate-install-assets` checks that the shipped user service remains user-scoped, the socket permissions stay private, the udev rule stays narrow, and the current polkit placeholder keeps denied defaults. `make smoke-uinput-status` verifies safe uinput/input-backend diagnostics and is included in `make verify`. Uinput access still relies on `/dev/uinput` being readable/writable by the daemon process.
...
- [x] Probe whether xdg-desktop-portal RemoteDesktop or libei can satisfy input needs before requiring uinput on the local machine. Current status: `seatgeist-cli input status` and MCP `seatgeist.input_backend_status` probe the user bus for `org.freedesktop.portal.RemoteDesktop`, KDE portal service visibility, libei client metadata/socket hints, and uinput fallback availability without starting a portal session. Status responses distinguish the configured backend request, the preferred visible backend, and the currently implemented input backend. `[backends].input`, `--input-backend`, and `SEATGEIST_INPUT_BACKEND` can request `auto`, `uinput`, `portal_remote_desktop`, or `libei`; raw keyboard/pointer commands route through a daemon input-executor trait. `auto` and `uinput` use the uinput executor today. Explicit `portal_remote_desktop` and `libei` selections build EIS action plans for text, named evdev key combos, and pointer requests, then execute through the stored daemon EIS session only after the per-plan readiness gate passes; without a stored session or ready selected device they fail closed before side effects.
...
- [~] Add release hardening. Current status: the canonical GitHub repository metadata, safe headless CI, dual license files, release checklist, and unsupported-path documentation are in place. `make verify-release-artifacts` builds a local binary/plugin archive, tracked-file source archive, manifest, and checksums, then verifies manifest coherence, checksums, bundle contents, and source contents. `make verify-release-install` extracts the generated bundle outside the working tree, runs packaged binary version checks, and validates packaged plugin/install assets. `make sign-release-artifacts` can create detached GPG signatures and a signature checksum manifest when `SEATGEIST_RELEASE_SIGNING_KEY` is set, and `make verify-release-signatures` verifies the generated signatures. GUI eval scripts write explicit `seatgeist_eval_evidence` JSON records on success. `make check-public-name` writes repeatable exact-name public registry evidence for crates.io, npm, PyPI, and GitHub exact names. `make release-readiness` reports current blockers across metadata, checklist, name-collision evidence, artifacts, signatures, and opt-in live eval evidence; `scripts/release-readiness.py --strict` is the final local no-blocker audit before upload. Signed release tags, public uploads, and broader manual release evidence remain incomplete.
