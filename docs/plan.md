...
- [~] Add release hardening. Current status: GitHub Actions runs the safe `make verify` gate on push and pull requests, `docs/release-checklist.md` records public-release blockers, `docs/unsupported-paths.md` documents unsupported public paths, and `make validate-release` enforces the dual license files plus CI/release anchors. `make verify-release-artifacts` builds a local binary/plugin archive, tracked-file source archive, manifest, and checksums, then verifies manifest coherence, checksums, bundle contents, and source contents. `make verify-release-install` extracts the generated bundle outside the working tree, runs packaged binary version checks, and validates packaged plugin/install assets. `make sign-release-artifacts` can create detached GPG signatures and a signature checksum manifest when `SEATGEIST_RELEASE_SIGNING_KEY` is set, and `make verify-release-signatures` verifies the generated signatures. GUI eval scripts now write explicit `seatgeist_eval_evidence` JSON records on success. `make check-public-name` writes repeatable exact-name public registry evidence for crates.io, npm, PyPI, and GitHub repository names. `make release-readiness` reports current blockers across metadata, checklist, name-collision evidence, artifacts, signatures, and opt-in live eval evidence, requiring matching `evidence.json` pass records for release evals; `scripts/release-readiness.py --strict` is the final local no-blocker audit before upload. Final public metadata, signed release tags, public uploads, and broader manual release evidence remain incomplete.
