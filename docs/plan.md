...
Eval 15: opt-in screenshot artifact metadata is correct. Current status: `make gui-eval-journal-artifacts` starts a private daemon with `[journal].include_artifact_metadata = true`, captures a bounded screenshot when a backend is available, and verifies the journal artifact path, byte count, and SHA-256 against the written PNG. It is intentionally opt-in because screenshot capture may request user consent.
```
