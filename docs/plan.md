...
- Add replayable action traces. Current status: `ReplayTrace` stores daemon requests with expected response metadata, `plasma-pilot-cli trace validate --file <path>` checks trace structure, version, and expected response types without contacting the daemon, `plasma-pilot-cli trace replay --file <path>` replays each step through the daemon so policy checks and journaling still apply, replay mismatches report the step index, label, method, and mismatch detail, and `examples/traces/status-smoke.json` provides a checked-in status-only replay fixture covered by the real CLI integration test and `make smoke-trace-replay`.
