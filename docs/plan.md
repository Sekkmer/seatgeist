...
- [x] Implement key combo and type text. Current status: `auto`/`uinput` supports US evdev ASCII text plus newline/tab and named key combos such as `Ctrl+L`; unsupported uinput text is rejected instead of guessed. Explicit stored EIS backends can use the text capability for UTF-8 text plans and model XKB-compatible text-keysym plans. The EIS crate now has a tested explicit-RMLVO xkbcommon keymap lookup that converts level-0 keysyms to evdev keycodes, while user-facing `key_combo` still uses named evdev codes until live KDE layout discovery and symbol-combo parsing are wired.
