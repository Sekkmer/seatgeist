...
- Add replayable action traces. Current status: `ReplayTrace` stores daemon requests with expected response metadata, `plasma-pilot-cli trace replay --file <path>` replays each step through the daemon so policy checks and journaling still apply, and `examples/traces/status-smoke.json` provides a checked-in status-only replay fixture covered by the real CLI integration test.
