...
- [x] Implement key combo and type text. Current status: `auto`/`uinput` supports US evdev ASCII text plus newline/tab and named key combos such as `Ctrl+L`; unsupported uinput text is rejected instead of guessed. Explicit stored EIS backends can use the text capability for UTF-8 text plans and model XKB-compatible text-keysym plans. Explicit EIS `key_combo` planning now keeps the named evdev parser first, then uses xkbcommon default-keymap lookup for unsupported single-character symbol parts such as `Ctrl+;`; live KDE layout discovery remains future work.
