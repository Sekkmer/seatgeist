...
- Add mock backends for unit tests. Current status: `plasma-pilot-testkit` provides deterministic screen, window, input, clipboard, and accessibility mocks with call recording; the input mock covers move, click point/button/count, drag, scroll, text, and key-combo events, and the accessibility mock covers focused-tree reads, find requests, invoke calls, set-text calls, insert-text calls, delete-text calls, copy-text calls, cut-text calls, and paste-text calls.
