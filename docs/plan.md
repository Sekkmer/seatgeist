...
- [x] Model the xdg-desktop-portal RemoteDesktop request/session contract before enabling execution. Current status: `plasma-pilot-portal` has tested constants and builders for `CreateSession`, `SelectDevices`, and `Start`, device-type and persist-mode validation, Request and Session handle-path derivation/validation, and parsers for session/start response metadata. This is contract-only; it does not yet start a portal RemoteDesktop session or send Notify*/EIS input events.
